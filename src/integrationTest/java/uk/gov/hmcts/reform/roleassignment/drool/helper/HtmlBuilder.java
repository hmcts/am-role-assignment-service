...
public class HtmlBuilder {
    ...
    public static final String CROSS = "&#10060;"; // The unicode character for cross
    ...
    public static String buildHtmlPageWithCollapse(String title, String body) {
        return buildHtmlPage(title, COLLAPSE_STYLE, title, body, COLLAPSE_SCRIPT);
    }

    public static String buildHtmlPage(String title, String body) {
        return buildHtmlPage(title, "", title, body, "");
    }
    ...
    public static String buildRelativeHyperLink(String currentPath, String targetPath, String linkText) {
        // Calculate the relative path from currentPath to targetPath
        Path current = Paths.get(currentPath);
        Path target = Paths.get(targetPath);
        Path relativePath = current.relativize(target);

        // Build the hyperlink HTML
        return HtmlBuilder.buildHyperlink(relativePath.toString(), linkText);
    }
    ...
    public static String buildHeading3(String text, String colour) {
        return String.format("<h3%s>%s</h3>", buildColorStyle(colour), text);
    }
    ...
    public static String buildCollapseDiv(String heading, String contents, boolean active) {
        return buildButton(
                getCollapseHeaderStyleClass(active),
                heading,
                getErrorColour(active)
        ) + buildDiv(
            getCollapseStyle(active),
            COLLAPSE_CONTENT_DIV_CLASS,
            contents
        );
    }
    ...
    public static String getErrorColour(boolean isError) {
