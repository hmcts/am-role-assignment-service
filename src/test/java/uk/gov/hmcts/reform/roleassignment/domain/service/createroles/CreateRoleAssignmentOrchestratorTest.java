...
class CreateRoleAssignmentOrchestratorTest {
    ...
    void createRoleAssignment_ReplaceTrue_RejectRoleRequests() throws Exception {
        ...
        HistoryEntity historyEntity = TestDataBuilder.buildHistoryIntoEntity(assignmentRequest.getRequestedRoles().iterator().next(), requestEntity);

        when(parseRequestService.parseRequest(any(AssignmentRequest.class), any(RequestType.class))).thenReturn(assignmentRequest);
