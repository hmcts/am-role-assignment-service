...
Feature: F-025 : CCD Case Disposer deletion tests
  ...
  @S-025.03
  @FeatureToggle(RAS:disposer_1_1=off)
  ...
  @S-025.06
  @FeatureToggle(RAS:disposer_1_1=off)
  Scenario: must successfully allow case-disposer to delete JUDICIAL case-roles
    Given a user with [an active IDAM profile with full permissions],
    And a user [Befta3 - who is the actor for requested role],
    And a successful call [to create org role assignments for actor (JUDICIAL) & requester (case-allocator)] as in [S-025.06_CreateOrgRoles_JUDICIAL],
    And a successful call [to create a case-role assignment for JUDICIAL] as in [S-025.06_CreateCaseRole_JUDICIAL],
    And a successful call [to check case role assignments are found for the actor] as in [F-025_CheckCaseRole__Found],
    When a request is prepared with appropriate values,
    And the request [with S2S token for case-disposer service],
    And the request [with process and reference values used to create the case-role assignment],
