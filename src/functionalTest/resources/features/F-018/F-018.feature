...
Feature: F-018 : Create Case Role Assignments for Employment Tribunal
  ...
  @S-269
  Scenario: must successfully create specific access granted role for ET judiciary
    Given a user with [an active IDAM profile with full permissions],
    And a user [Befta1 - who is the actor for requested role],
    And a successful call [to create org role assignments for actors & requester] as in [S-269_Org_Role_Creation],
    And a successful call [to create role assignments for requested role] as in [S-269_Access_Requested],
    When a request is prepared with appropriate values,
    And the request [contains specific-access-judiciary case granted role assignment],
    And it is submitted to call the [Create Role Assignments] operation of [Role Assignments Service],
    Then a positive response is received,
    And the response has all other details as expected.
    And a successful call [to delete role assignments just created above] as in [S-269_DeleteDataForRoleAssignmentsForGrantedRole],
