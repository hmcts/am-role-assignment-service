...
Feature: F-013 : Create SSCS Role Assignments
 ...
 @S-234
  ...
  Scenario: must successfully create SSCS CASE role assignment with all mandatory fields
    ...
    And the response has all other details as expected.
       And a successful call [to delete role assignments just created above] as in [DeleteDataForRoleAssignments],
       And a successful call [to delete role assignments just created above] as in [S-234_DeleteDataForRoleAssignmentsForOrgRoles],
       And a successful call [to delete role assignments just created above] as in [S-234_DeleteDataForRoleAssignmentsForRequestedRole].
 ...
 @S-235
  ...
  Scenario: must successfully create SSCS CASE role assignment with all mandatory fields
    ...
    And the response has all other details as expected.
    And a successful call [to delete role assignments just created above] as in [DeleteDataForRoleAssignments],
    And a successful call [to delete role assignments just created above] as in [S-235_DeleteDataForRoleAssignmentsForOrgRoles],
    And a successful call [to delete role assignments just created above] as in [S-235_DeleteDataForRoleAssignmentsForRequestedRole].
