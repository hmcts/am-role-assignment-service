...
Feature: F-022 : Create Case Role Assignments for POSSESSIONS STAFF case roles
  ...
  @S-022.12 @Ignore
      Scenario: must successfully create case-allocator POSSESSIONS case case-allocator Enforcement role
        Given a user with [an active IDAM profile with full permissions],
        And a user [Befta3 - who is the actor for requested role],
        And a successful call [to create org role assignments for actors & requester] as in [S-022.12_Org_Role_Creation],
        When a request is prepared with appropriate values,
        And the request [contains ReplaceExisting is false and reference set to caseId],
        ...
        And a successful call [to delete role assignments just created above] as in [S-022.12_DeleteDataForRoleAssignmentsForOrgRoles].

  @S-022.12a
      Scenario: must successfully create case-allocator POSSESSIONS case case-allocator CTSC role
        Given a user with [an active IDAM profile with full permissions],
        And a user [Befta3 - who is the actor for requested role],
        And a successful call [to create org role assignments for actors & requester] as in [S-022.12a_Org_Role_Creation],
        When a request is prepared with appropriate values,
        And the request [contains ReplaceExisting is false and reference set to caseId],
        And the request [contains case-allocator case role assignment],
        And it is submitted to call the [Create Role Assignments] operation of [Role Assignments Service],
        ...
        And the response has all other details as expected,
        And a successful call [to delete case role assignment for the same actor] as in [S-022.12_Delete_Case_Role],
        And a successful call [to delete role assignments just created above] as in [S-022.12a_DeleteDataForRoleAssignmentsForOrgRoles].
