...
Feature: F-017 : Create Role Assignments for Group Access
  ...
  @S-250.GA
  Scenario: Invoking POST role-assignments api with request to add a new role assignment (PRM GA Role)
  ...
  @S-250.Org
  Scenario: Invoking POST role-assignments api with request to add a new role assignment (PRM Org Role)
    Given an appropriate test context as detailed in the test data source,
    And a user [Befta6 - who invokes the API],
    When a request is prepared with appropriate values,
    And the request [has roleType Organisation, roleCategory Professional, roleName PRM_Test_Org_Role and attributes has caseType but not caseAccessGroupId],
    And it is submitted to call the [Create Role Assignments] operation of [Role Assignments Service],
    Then a positive response is received,
    And the response has all other details as expected,
    And a successful call [to delete role assignments just created above] as in [S-250_DeleteDataForRoleAssignmentsForPrmRoles].
