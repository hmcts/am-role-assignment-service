...
Feature: F-021 : Create Case Role Assignments for POSSESSIONS JUDICIAL case roles
  ...
  Scenario: must successfully create case-allocator POSSESSIONS case role with existing org role - case-allocator ADMIN role
    Given a user with [an active IDAM profile with full permissions],
    And a user [Befta3 - who is the actor for requested role],
    And a successful call [to create org role assignments for actors & requester] as in [S-021.04_Org_Role_Creation],
    When a request is prepared with appropriate values,
    And the request [contains ReplaceExisting is false and reference set to caseId],
    ...
    Then a positive response is received,
    And the response has all other details as expected,
    And a successful call [to delete case role assignment for the same actor] as in [S-021.04_Delete_Case_Role],
    And a successful call [to delete role assignments just created above] as in [S-021.04_DeleteDataForRoleAssignmentsForOrgRoles].
