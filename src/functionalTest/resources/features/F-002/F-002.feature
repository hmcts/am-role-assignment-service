...
Feature: F-002 : Delete Role Assignments by Actor Id
  ...
  @S-027
  Scenario: must receive an error when delete Role Assignment with a non-existing Process
    Given a user with [an active IDAM profile with full permissions],
    When a request is prepared with appropriate values,
  ...
  @S-028
  Scenario: must receive an error when delete Role Assignment with a non-existing Reference
    Given a user with [an active IDAM profile with full permissions],
    When a request is prepared with appropriate values,
