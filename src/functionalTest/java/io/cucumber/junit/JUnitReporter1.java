...
import io.cucumber.plugin.event.Result2;
...
final class JUnitReporter1 {
    ...
    JUnitReporter1(EventBus bus, JUnitOptions junitOption) {
    ...
    private void handleStepResult(PickleStepTestStep testStep, Result2 result) {
    ...
    private void handleTestCaseResult(TestCaseFinished event) {
        Result2 result = event.getResult();
        switch (result.getStatus()) {
            case PASSED:
                // do nothing
                break;
...
/*if (stepErrors.isEmpty()) {
    ...
    private void handleHookResult(Result2 result) {
